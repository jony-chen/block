  ...
  <div class="main">
    <section class="main_body">
      ...
      <div class="main_box">
          ...
          <div class="successUpload">
            ...
            >
              ...
              <dd>
                ...
                <p>{{ totalSumAll + untotalSumAll }}<span>pcs</span></p>
              </dd>
              <dd>
              ...
              <!-- <download-excel
                v-if="output"
                class="export-excel-wrapper export"
                :data="DetailsForm"
                :fields="json_fields"
                :header="title"
                name="零件列表.xls"
        ...
        <div class="shopcart_box" v-if="inForm.length != 0">
          <div class="shoppCartContent" @click="addshopcart">
...
export default {
  ...
  computed: {
    ...
    untotalSumAll() {
      let totalSumAll = 0;
      if (this.unForm.length != 0) {
        for (let i = 0; i < this.unForm.length; i++) {
          totalSumAll += this.unForm[i].Quantity;
        }
      }
      if (isNaN(totalSumAll)) {
        return 0;
  ...
  methods: {
    ...
    async dealNum(item) {
      ...
      //excal数据
      let index = item.indexOf("1"); //找出第一个有1的地方的位置
      let arr = item.substring(index); //从有1的地方开始取
      // let index1 = item.indexOf("0 FILE "); //找出第一个有0 FILE的地方的位置（0 file做开头跟结尾）
      let partarr = arr.split("0 FILE "); //前面部分是零件，后面是零件组
      if (partarr.length > 1) {
        //判断是否含有数组
        var arrarr = [];
        for (var j = 1; j < partarr.length; j++) {
          let arr = partarr[j].split("\r");
          let newarr = this.getdata(arr, 0);
          arrarr.push(newarr);
        }
        this.nowdata(arrarr);
        // let arrobjresult = [];
      }
      let arralldata = partarr[0].split("\r");
      let allnewarr = this.getdata(arralldata, 1);
      // for(let z = 0;z < arrarr.length;z++) {
      //   allnewarr.push(arrarr[z]);
      // }
      // arrobjresult.push(allnewarr);
      let arrresult = this.redata(allnewarr, arrarr);
      for (let i = 0; i < arrresult.length; i++) {
        for (let j = i + 1; j < arrresult.length; j++) {
          if (
            arrresult[i].Part == arrresult[j].Part &&
            arrresult[i].Color == arrresult[j].Color
          ) {
            arrresult[i].Quantity =
              arrresult[i].Quantity + arrresult[j].Quantity;
            arrresult.splice(j, 1);
            j--;
          }
        }
      }
      this.fileKind = arrresult.length;
      this.DetailsForm = arrresult;
      if (this.DetailsForm.length != 0) {
        this.output = true;
      }
      try {
        let searchdata = await Home.uploadLdr(this.DetailsForm);
      ...
      // if (arrresult.length < 5004) {
      // } else {
      //   this.$message.error("因系统限制，单次上传订单零件不要超过5000个!");
      // }
    },
    getdata(arr, num) {
      var newarr = [];
      ...
      for (var i = 0; i < arr.length; i++) {
        // if(rule.exec(arr[i])) {
        if (arr[i].indexOf("dat") != -1) {
          let Part = arr[i].substring(
            arr[i].lastIndexOf(" ") + 1,
            arr[i].length - 4
          );
          let Quantity = 1;
          // let Color = arr[i].substring(arr[i].indexOf(" ") + 1, arr[i].lastIndexOf(" ") + 1);
          let fristindex = arr[i].indexOf(" ") + 1;
          let lastindex = arr[i].indexOf(" ", fristindex);
          // let colorindex = fristindex + lastindex;
          let Color = arr[i].substring(fristindex, lastindex);
          // let image = "https://pic-1302893429.cos.ap-nanjing.myqcloud.com/items/" + Color + "/" + Part + "_" + Color + ".png";
          let rowdata = {
            Part: Part,
            // image: image,
            Quantity: Quantity,
          ...
          if (arr[i].length > 50) {
            //判断数组的数量
            let arrindex = this.getindexfind(arr[i], " ", 13);
            let arrname = arr[i].substring(arrindex + 1, arr[i].length);
            newarr.push(arrname);
          }
        }
      }
    ...
    nowdata(arrarr) {
      for (let k = 0; k < arrarr.length; k++) {
        for (let m = 0; m < arrarr[k].length; m++) {
          if (typeof arrarr[k][m] == "string") {
            for (let d = 0; d < arrarr.length; d++) {
              if (
                arrarr[k][m].toLowerCase() == arrarr[d][0].arrname.toLowerCase()
              ) {
                arrarr[k].splice(m, 1);
                for (let c = 1; c < arrarr[d].length; c++) {
                  arrarr[k].push(arrarr[d][c]);
                }
              }
            }
          }
    ...
    redata(allnewarr, arrarr) {
      for (let i = 0; i < allnewarr.length; i++) {
        if (typeof allnewarr[i] == "string") {
          for (var k = 0; k < arrarr.length; k++) {
            if (
              allnewarr[i].toLowerCase() == arrarr[k][0].arrname.toLowerCase()
            ) {
            }
            allnewarr.splice(i, 1);
            for (let c = 1; c < arrarr[k].length; c++) {
              allnewarr.push(arrarr[k][c]);
            }
          }
        }
      }
    ...
    getindexfind(str, cha, num) {
      //参数（字符串，要查找的字符串值，要查找第⼏个该字符串值)
