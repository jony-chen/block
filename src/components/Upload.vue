  ...
  <div class="main">
    <section class="main_body">
      <header class="head d-flex align-items-center justify-content-space-between">
        ...
        <div class="head_btn d-flex align-items-center justify-content-space-between " v-if="!nowlogin">
          <p @click="loginurl">登录</p>
          <p @click="loginurl">注册</p>
          <p @click="remind">上传</p>
        </div>
        <div class="head_name d-flex align-items-center justify-content-space-between " v-if="nowlogin">
          <dl>欢迎您，</dl>
          <router-link to="/myorder"><dd>{{uname}}</dd></router-link>
          <p @click="exit">退出</p>
          <div class="head_btn" @click="upldr">
            <p>上传</p>
          </div>
        </div>
      </header>
      <div class="main_box">
        ...
        <div class="shopcart_box" @click="addshopcart" v-if="inForm.length != 0">
          <div class="shoppCartContent">
            <div class="addToCat floatR"
              ><span class="totalPrice">￥ {{this.inForm.length * 0.5}}</span>
              <i class="el-icon-document"></i>
        ...
        <div class="part_box" v-if="output">
          ...
          <div class="block_box" v-for="(item,id) in this.inForm" :key="id">
            <div class="part_img">
              <img :src="item.in == 1 && item.type == 1 ? 'https://pic-1302893429.cos.ap-nanjing.myqcloud.com/items/' + item.Color + '/' + item.Part + '_' + item.Color + '.png' : '../assets/images/default.png'" @error="defaultSrc"/>
            </div>
...
export default {
  ...
  methods: {
    ...
    async dealNum(item) {
      ...
      //excal数据
      let arr = item.split("\r");
      if (arr.length < 5004) {
        let newarr = [];
        // console.log(arr[8].indexOf('dat'));
        for (var i = 0; i < arr.length; i++) {
          // if(rule.exec(arr[i])) {
          if (arr[i].indexOf("dat") != -1) {
            // console.log(rule.exec(arr[i]));
            let Part = arr[i].substring(
              arr[i].lastIndexOf(" ") + 1,
              arr[i].length - 4
            );
            let Quantity = arr[i].substring(1, 2);
            // let Color = arr[i].substring(arr[i].indexOf(" ") + 1, arr[i].lastIndexOf(" ") + 1);
            let fristindex = arr[i].indexOf(" ");
            let lastindex = arr[i].indexOf(" ", fristindex + 1);
            // console.log(lastindex);
            let colorindex = fristindex + lastindex;
            let Color = arr[i].substring(fristindex + 1, colorindex - 2);
    ...
    // async uploadLdr(data){
    //   // console.log(this.DetailsForm);
    //   let searchdata = await Home.uploadLdr(data);
    //   return searchdata;
    // }
  },
};
